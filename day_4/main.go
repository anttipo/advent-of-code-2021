...
func main() {
	...
	var bingoNumbers []int16
	data, err := ioutil.ReadFile("input")
	if err != nil {
		log.Fatal(err)
	}

	rawNums := strings.Split(string(data), "\n\n")
	fmt.Println(rawNums)
	numbersAux := strings.Split(rawNums[0], ",")
	fmt.Println(numbersAux)

	for _, value := range numbersAux {
		val, _ := strconv.ParseInt(value, 10, 16)
		bingoNumbers = append(bingoNumbers, int16(val))
	}
	...
	sol1 := part1(bingoNumbers, bingos)
	sol2 := part2(bingoNumbers, bingos)
